...
**Note on composition edges:** The "Draws on" skills describe the *methodology* applied within the lazy-loop phases. Each phase receives structured outputs from the prior phase, providing per-phase observability. The composition with pragmatic-semantics, pragmatic-cybernetics, essentialist, and grill-me is methodological guidance.
...
- **Invoked from other skills:** `essentialist` can call the full loop when deletion test results are ambiguous.
...
- **Threshold:** 0.25 (converged when stable least-action recommendation is found)
- **Max iterations:** 3
